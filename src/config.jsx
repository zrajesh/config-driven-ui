...
// renderer function which accepts the config and renders different components 
// accordingly.
export const CustomElementRenderer = ({ obj, onChangeFunc, value = null }) => {
    switch (obj.component) {
        ...
        case "submit":
            return (
                <input type={"submit"} value={obj.buttonValue} className={`${obj.className}`} disabled={obj.disabled} style={obj.style} />
            );
        case "dropdown":
            return (
                <div>
                    <label>{obj.label}</label>
                    <select className={`${obj.className}`} value={obj.value} onChange={(e) => onChangeFunc(obj.name, e.target.value)}>
